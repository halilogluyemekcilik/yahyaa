const CACHE_NAME = 'hukuk-takip-v1.0.3'; // Versiyon artırıldı
...
self.addEventListener('install', event => {
  console.log('🚀 Hukuk SW kurulumu başlıyor...');
  self.skipWaiting();
  ...
  event.waitUntil((async () => {
    ...
    console.log('🎉 Hukuk SW kurulumu tamamlandı!');
  })());
});

self.addEventListener('activate', event => {
  console.log('🔄 Hukuk SW aktifleştiriliyor...');
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.map(key => {
      if (key.startsWith('hukuk-takip-') && key !== CACHE_NAME) {
        console.log('🗑️ Eski hukuk cache siliniyor:', key);
        return caches.delete(key);
      }
    }));
    
    await clients.claim();
    console.log('✅ Hukuk SW aktifleştirildi!');
  })());
});
...
self.addEventListener('fetch', event => {
  ...
  if (!request.url.includes('/game/hmgsSinavTakipSistemi/')) {
...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification('Hukuk Takip Sistemi', options)
  );
});
