...
// 所有环境配置
// Tüm çevresel unsurların (zemin, binalar, engeller, coin'ler, ışıklar) kurulumu
export default class Environment {
    ...
    // Oyunun başlangıç ortamı kurulumları
    startGame() {
        this.plane = [];
        this.obstacal = [];
        this.z = -1 * (roadLength / 2) + 10;
    ...
    setGroupScene(z: number, houseZ: number, isloadAgain: boolean) {
        ...
        // Sağ/sol binaları yerleştir
        this.loadmodelAndSize(modelGroup, houseZ, isloadAgain);
        // Engelleri ve coin'leri yerleştir
    ...
    async setPlane(modelGroup: THREE.Group, z: number) {
        ...
        // Ana yol ve yan zemin dokularını ayarla
    ...
    async loadObstacle(modelGroup: THREE.Group, houseZ: number) {
        // Çarpışma kontrolüne dahil olan grup
        const obstacalGroup = new THREE.Group();
        // Görsel ama çarpışmaya dahil olmayan grup (örn. coin ve yardımcı meshler)
        ...
        // Tren ve engeller için ölçek-doku-ışık ayarları
        ...
        let i = -1;
        let increase = true;
        // Statik trenleri şeritlere yerleştir
        ...
        let j = -1;
        let increase1 = true;
        // Statik yol bariyerlerini yerleştir
    ...
    async loadmodelAndSize(modelGroup: THREE.Group, houseZ: number, load: boolean) {
        if (load) {
            ...
            console.log('Bina modelleri yüklendi');
    ...
    comupteBox(scene: THREE.Group) {
        // Modelin axis-aligned bounding box (AABB) hesaplaması
        const boundingBox = new THREE.Box3().setFromObject(scene);
        // Boyutları al
        const size = new THREE.Vector3();
        boundingBox.getSize(size);
        const modelWidth = size.x; // model genişliği
        const modelHeight = size.y; // model yüksekliği
        const modelDepth = size.z; // model derinliği
        const modelSize = {
            x: modelWidth,
            y: modelHeight,
            z: modelDepth,
