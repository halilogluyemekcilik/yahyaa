<template>
  <div>
    <div v-if="!isReady" class="loading">
      ...
      <div>Kaynak yükleniyor: {{ loadingData.url }}</div>
      <div>Yüklendi {{ loadingData.itemsLoaded || 0 }}/{{ loadingData.itemsTotal || 0 }}</div>
      <div v-if="loadingData.type === 'successLoad'">Yükleme başarılı, lütfen bekleyin</div>
    </div>
...
// Modellerin/varlıkların yüklenme durumunu takip eder
const isReady = ref(false);
// Skor
const score = ref(0);
// Toplanan coin sayısı
const coin = ref(0);
// Küçük hata sayacı (ör. çarpma, takılma vb.)
const mistake = ref(0);
// Oyun durumu (ready | start | pause | over ...)
...
onMounted(() => {
  // Oyun motorunu başlat ve canvas referansını ilet
  const game = new Game(exp_canvas.value);
  // Kaynak yükleme ilerlemesini dinle ve yükleme overlay'ini yönet
  ...
  // Oyun durumunu (başladı/durdu/bitti) dinle
  game.on('gameStatus', (data: any) => {
    console.log(data);
    gameStatus.value = data;
  });
  // Skor/coin/hata güncellemelerini dinle ve panellere yansıt
...
onUnmounted(() => {
  // Bileşen yok olurken oyunu ve kaynakları temizle
  const game = new Game(exp_canvas.value);
  game?.disposeGame();
});
</script>
