...
function pointerDown(e) {
    ...
    dragStart = { tile, x: e.clientX, y: e.clientY };
...
function animateOneWaySwap(tile1, tile2, classes) {
    return new Promise((resolve) => {
        let finished = 0;
        const endA = () => {
            tile1.classList.remove(classes.a);
            finished++;
            if (finished === 2) resolve();
        };
        const endB = () => {
            tile2.classList.remove(classes.b);
            finished++;
            if (finished === 2) resolve();
        };
        tile1.addEventListener("animationend", endA, { once: true });
        tile2.addEventListener("animationend", endB, { once: true });
...
function cleanupSparkles(tile) {
    const sparks = tile.querySelectorAll('.sparkle');
    sparks.forEach((n) => n.remove());
}
